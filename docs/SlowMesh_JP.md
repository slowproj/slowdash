...
### Persistency 
レジストリの指定のパスを persistent と指定することにより，そのパスおよびそれ以下のパスへの書き込みが起こる度に，Persistent パス以下全体をファイルに保存するようにできます．作成されるファイル名は `registry-{path}.json` で，その中に，JSON フォーマットで，フラットな１エントリ１キーで `{subpath}: {record}` の形式で書き出されます．（`{path}.>` 形式で木構造化しないのは，dict 値を保持するエントリの形を保存するためです．） 書き出されたレジストリ値は，次にレジストリサーバー（SlowDash サーバー）がスタートしたとき，自動で読み込まれます．

TODO: 現時点で，CAS などの値以外のメタ情報は保存はされますが，読み込みはされません．
...
また，WebMesh はメッシュ上に流れるデータパケット（`data.*.>` トピック）のチャンネルごとの最新値をキャッシュし，HTTP のデータクエリに対して，他のコンポーネントがデータを返さない場合，キャッシュされたデータを応答します．ただし，時系列データはキャッシュされていても使われません．
...
同じスクリプトを複数走らせたい場合，`SlowdashProject.yaml` ファイル内に同じスクリプトで名前の異なる複数のエントリを作ることができます．

多数のタスクを同時に自動で走らせたい場合は，`slowdash-task` コマンドを `--name=NAME` オプション付きで多数実行するような Python スクリプト (Spawn Script) を作成し，`SlowdashProject.yaml` からはそれだけを起動するようにすることもできます．
この構成では，Spawn スクリプトの中で初期値の設定や SubPub や RPC によるコマンドの分配も細かく指定できるようになります．
`slowdash-task` コマンドは，それを起動した親プロセスが終了すると自分も自動で終了するので，Spawn スクリプトが終了すればそこから起動されたタスクはすべて自動終了します．
...
#### GET `api/data/{*}`
`data.*` トピックに流れているデータのチャンネルごとの最終値をキャッシュしたもの．
このクエリに他のコンポーネントがデータを返さず，かつ，キャッシュデータのタイムスタンプがクエリ範囲の場合のみ，キャッシュの最終値が挿入される．
