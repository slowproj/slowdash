...
モニタタスクは，intent の consequences を呼んで，正常条件を更新し，その後指定時間以内に届くデータと比較します．
...
あと，監視データ上にエラーが出たときに，どの操作によるものかの対応付けができます．これは，最近流行の Observability/Tracability に似ていますが，ハードウェアを通してもトレースをつなげることができます．（通常の Observability で使うような，Trace-ID / Span-ID を付加するような方法は，デバイスを通過しての操作ー結果の因果関係を追跡できません．）
