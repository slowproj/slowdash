...
- Jupyter に接続して収集中のデータを対話的に解析する

ための Web-Python ベースのソフトウェアです．ブラウザ側の JavaScript と，サーバー側の Python で構成されています．
...
- 表示プロットを Jupyter にエクスポートして解析を継続できる
...
ここまでを行った例が `ExampleProjects/QuickTour/02_RealDevice` にあります．
...
```
データクラス `run_status` で，現在のランの状態を管理します．この後，ここにいろいろな状態変数を追加していきますが，今は「動作中」を示すフラグ `running` だけです．メインのループでこの変数を見て，デバイスからデータを読み出すかを決めます．また，この変数を外部から設定するための関数 `start()` と `stop()` を作成します．
...
```
`sd-enabled` 属性でよく使われる他の Data Transform としては，数値の大小を判定する `->gt(thresh)` / `->lt(thresh)` や，文字列に対する `->match(pattern)` などがあります．
