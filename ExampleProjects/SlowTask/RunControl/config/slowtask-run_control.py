...
async def _loop():    
    ...
    await ctrl.aio_emit(run_status)
...
async def start(run_number:int=None, stop_after:bool=None, run_length:float=None, repeat: bool=None, offline:bool=None):
    ...
    await ctrl.aio_emit(run_setting)
    await ctrl.aio_emit(run_status)
    ...
    for i, ev in enumerate(events):
        ...
        if i == 0:
            for channel, value in hits.items():
                await ctrl.aio_emit(value, name=channel)

    await ctrl.aio_emit(rate_trend.timeseries(), name='rate_trend')
    await ctrl.aio_emit(nhits_hist, name='nhits_hist')
