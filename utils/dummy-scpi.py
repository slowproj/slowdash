...
if __name__ == '__main__':
    ...
    device = sc.DummyScpiDevice_RandomWalk(line_terminator='\x0a' if opts.line_terminator=='LF' else '\x0d')
    server = sc.SerialDeviceEthernetServer(device, port=opts.port)
    server.start()
